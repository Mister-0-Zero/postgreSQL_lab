...
BEGIN
  FOR i IN 1..4 LOOP
	  FOR d IN SELECT id FROM deceased ORDER BY random() LIMIT 3000 LOOP
	    -- случайное отношение и пол
	    ...
	    FROM (
	      VALUES 
	        ...
	        ('муж', 'male'),
	        ('жена', 'female')
	    ) AS t(relationship, gender)
	    ORDER BY random()
	    LIMIT 1;
	
	    -- выбираем случайное ФИО
	    IF rel.gender = 'male' THEN
	      SELECT
	        lnames[ceil(random() * array_length(lnames,1))::int],
	        fnames[ceil(random() * array_length(fnames,1))::int],
	        patronymics[ceil(random() * array_length(patronymics,1))::int]
	      INTO lname, fname, patronymic
	      FROM (
	        SELECT ARRAY[
			    'Александр', 'Максим', 'Иван', 'Артём', 'Дмитрий', 'Никита', 'Михаил', 'Даниил',
			    'Егор', 'Андрей', 'Сергей', 'Владимир', 'Олег', 'Юрий', 'Константин', 'Павел',
	    ...
	    phone := '+7' || lpad(trunc(random() * 1e10)::text, 10, '0');
	
	    INSERT INTO relatives(full_name, phone, relationship, deceased_id)
	    VALUES (full_name, phone, rel.relationship, d.id);
	  END LOOP;
  END LOOP;
END $$;
...
DECLARE
  ...
  random_status TEXT;
BEGIN
  FOR i IN 5434..11923 LOOP
    -- Получаем случайный service_id
    SELECT id INTO random_service_id FROM services WHERE id <> 1 ORDER BY random() LIMIT 1;
    ...
    -- Выбираем дату заказа до 5 дней от даты смерти
    random_order_date := death_date + (trunc(random() * 5))::int; 
